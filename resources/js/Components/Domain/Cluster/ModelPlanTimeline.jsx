            ...
            {items.map((item, index) => (
                <div key={index} className="relative pb-6 sm:pb-8 last:pb-0">
                    ...
                    {index < items.length - 1 && (
                        <div className="absolute left-5 top-10 bottom-0 w-0.5 bg-gradient-to-b from-blue-300 to-gray-200" aria-hidden="true"></div>
                    )}

                    {/* スポット情報 */}
                    <div className="flex items-start gap-3 sm:gap-4">
                        {/* 番号アイコン */}
                        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 text-white flex items-center justify-center font-bold text-sm shadow-md relative z-10 ring-4 ring-white">
                            {index + 1}
                        </div>

                        {/* コンテンツカード */}
                        <div className="flex-1 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 p-4 sm:p-5">
