...
2. **提案の待機:** 事前に完了しているAI分析結果を基にリアルタイムで提案を生成。待ち時間は1分未満に抑え、ストレスの少ない体験を提供します。
...
- **提案待機ページ:** ユーザーごとに一意のURLを生成。事前のAI分析結果を基に、ユーザーの好みに合わせた提案を短時間（1分未満）で生成。待機中はシンプルなローディング表示でストレスを軽減。
...
- **アーキテクチャ:** 開発速度を優先するMVPフェーズでは`Laravel 12` + `Inertia.js 2` + `React 19`のモノリシック構成を採用。将来的にAI機能を強化するフェーズで、`FastAPI`(Python)によるAIエンジンをマイクロサービスとして分離する段階的アプローチを採る。
- **データベース:** `PostgreSQL 16` + `PostGIS 3.4`を採用。スポット情報を最小単位とする「スポット中心アプローチ」で、柔軟なデータ構造を実現。
- **テスト:** MVPフェーズの単体テストには`Pest 4`を使用。
- **AI統合:** Google Gemini APIを活用し、複数のAIモデルを性能優先度に基づいて動的に選択。

詳細な技術仕様（システムアーキテクチャ、アーキテクチャ決定記録、データベース設計）については、`documents/2_technical_spec.md`を参照してください。
...
- **2フェーズアーキテクチャによる高速化とコスト抑制:**
  - **Phase 1（非同期AI分析）:** スポット詳細、優先度、画像選定、キャッチフレーズ、モデルプラン生成などの重い分析処理を、バックグラウンドで事前に実行・蓄積。Laravelのキューシステムで段階的に処理。
  - **Phase 2（リアルタイム提案生成）:** ユーザーからのリクエスト時は、Phase 1で蓄積済みのデータを組み合わせて高速に提案を生成。待ち時間を3-5分から1分未満に短縮し、優れたUXを実現。
  - これにより、高価なAI APIコールは事前バッチ処理に集約され、ユーザーリクエスト時のコストを最小化。匿名ユーザーの行動はセッションで管理し、会員登録時にアカウントへ紐付けます。
...
- **MVP (〜2週間): コア体験の魅力検証**
    ...
    - **コア体験:** 「出発地を入力すると、短時間（1分未満）で視覚的に魅力的な複数の旅行プランが提示される。各プランの詳細（モデルコース、スポット情報）を閲覧し、旅のイメージを広げることができる」という一連のUXフローを実現します。事前のAI分析により待ち時間を最小化し、ストレスのない体験を提供します。
    - **スコープ:**
